<?php
    if($CONTACT==true){
        $sql = "SELECT COUNT(*) as numb FROM table_contact where view=0 ";
        $row_lienhe = $db->rawQueryOne($sql);
    }
    if($ORDER==true){
        $sql = "SELECT COUNT(*) as numb FROM table_order where view=0 ";
        $row_giohang = $db->rawQueryOne($sql);
    }
    if($NEWSLETTER==true){
        $sql = "SELECT COUNT(*) as num FROM table_newsletter ";
        $row_dangky = $db->rawQueryOne($sql);
    }
    if($BOOKING==true){
        $sql = "SELECT COUNT(*) as numb FROM table_booking ";
                ...
                <li class="ddnew" style="position:relative">
                    ...
                    <div class="popover-notifications" style="top: 35px;right: 0px;transform-origin: 60px top;">
                        <div>
                            ...
                            <div>
                                <a href="index.html?com=newsletter&act=man&type=ho-tro" title=""
