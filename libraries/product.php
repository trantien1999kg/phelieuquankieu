    ...
    class product{
        ...
        public function getMan(){
            ...
            if($this->_table == 'flashsale_info'){
                ...
                $idp = (int)($_GET['id_product']);

                $idf = (int)($_GET['id_flash']);

                $sql = "select * from #_{$this->_table} where id_flash=? and id_product=?";

                $item = $this->_d->rawQueryOne($sql,array($idf,$idp));
            ...
            if(!in_array($this->_com,$this->_allowed)){
                ...
                if($act == 'edit_list' && $type=='san-pham'){

                    $ds_photo = $this->_d->rawQuery("select * from #_{$this->_temp_table} where id_list=? and type=? order by stt asc, id desc ",array($id,$type));
        ...
        public function saveMan(){
            ...
            if(isset($data["khuyenmai"])){
                ...
                $send["khuyenmai"] = json_encode($data['khuyenmai']);
           
            }

            if(isset($data["id_size"])){

                $send["id_size"] = implode(',',$data['id_size']);
            ...
            if(!empty($file4)){
                ...
                if($id){
                    ...
                    if($file4['error']==0){
                        ...
                        $photo = $this->_func->uploadImg($id,"banner","",$file4,$folder,$this->_com,$table['img-width'],$table['img-height'],$table['img-ratio'],$table['img-b']);

                        

                        $send['banner'] = $photo['banner'];
                    ...
                    if($file4['error']==0){
                        ...
                        $photo = $this->_func->uploadImg(0,"banner","",$file4,$folder,$this->_com,$table['img-width'],$table['img-height'],$table['img-ratio'],$table['img-b']);
            ...
            if($data['soluongban']){
                ...
                $send['soluongban']=str_replace(',','',$data['soluongban']);
            ...
            if($data['point']){
                $send['point']=str_replace(',','',$data['point']);
            }
            ...
            if(isset($_POST['time'])){
                ...
                if(is_array($time)){
                    $time_start = $data['date']." ".$time[0];
                    $time_end = $data['date']." ".$time[1];         
                    $send['time_start'] = strtotime($time_start);
                    $send['time_end'] = strtotime($time_end);
                }
            }
                        ...
                        if (isset($_FILES['files'])) {
                            ...
                            for($i=0;$i<count($_FILES['files']['name']);$i++){
                                ...
                                if($_FILES['files']['name'][$i]!=''){
                                    ...
                                    if(isset($_POST["linkhinh"][$i])){

                                        $sendx['link'] = $_POST["linkhinh"][$i];
        ...
        public function deleteManFlashsale(){
            ...
            if(isset($_GET['id'])){
                ...
                if($item){
                    ...
                    var_dump($this->_d->getLastError());die();
        ...
        public function deleteManFlashInfo(){

            global $type,$url_path,$folder;

            
            if(isset($_GET['id_product_flash'])){

                $id =  (int)$_GET['id_product_flash'];
                
                $id_flash = (int)$_GET["id_flash"];

                $item=$this->_d->rawQueryOne("select id_product from #_{$this->_table} where id_product=? and id_flash=?",array($id,$id_flash));

                if($item){

                   
                    $this->_d->where('id_product', $item['id_product']);
                ...
                if(count($listid)){
                    ...
                    foreach ($listid as $k => $v) {
                        ...
                        $item=$this->_d->rawQueryOne("select id_product from #_{$this->_table} where id_product=?",array($id));
