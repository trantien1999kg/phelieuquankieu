<?php  if(!defined('_source')) die("Error");
    ...
    if(!empty($idl)){
        ...
        $product_tags = $db->rawQueryOne("select id,ten_$lang,tenkhongdau_$lang as tenkhongdau,mota_$lang,photo,title,keywords,description,type from #_tags where id=? and type=? and hienthi=1 order by stt asc",array($idl,$type));
        ...
        if($product_tags){
            ...
            $tintuc=$db->rawQuery("select *,tenkhongdau_$lang as tenkhongdau , ten_$lang as ten from #_baiviet where hienthi=1 {$subWhere} and type=? $order_by limit $startpoint,$per_page",array('san-pham'));
            ...
            $count=$db->rawQueryOne("select COUNT(*) as `numb` from #_baiviet where hienthi=1 {$subWhere} and type=?", array($type));
    ...
    //         $tintuc=$db->rawQuery("select *,tenkhongdau_$lang as tenkhongdau from #_baiviet where hienthi=1 and noibat=1 and type=? $order_by limit $startpoint,$per_page",array('san-pham'));
    //         $count=$db->rawQueryOne("select COUNT(*) as `numb` from #_baiviet where hienthi=1 and type=?", array($type));
       
    //     }elseif($com=='tin-tuc-noi-bat'){
    //         $tintuc=$db->rawQuery("select *,tenkhongdau_$lang as tenkhongdau from #_baiviet where hienthi=1 and noibat=1 and type=? $order_by limit $startpoint,$per_page",array('tin-tuc'));
    //         $count=$db->rawQueryOne("select COUNT(*) as `numb` from #_baiviet where hienthi=1 and type=?", array($type));
    //     }else{
    ...
    //     $post_tags = $db->rawQuery("select id,ten_$lang as ten,tenkhongdau_$lang as tenkhongdau from #_tags where hienthi=1 and id in ($im_tag) and type=? order by stt asc",array($type));
